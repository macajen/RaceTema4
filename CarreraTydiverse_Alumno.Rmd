...
author: "Jenny Carolina Matamoros"
...
```{r}
#transformamos los datos en un tibble y los almacenamos en una variable llamada cran
cran <- as_tibble(mydf)
#se usa rm para eliminar el data frame original llamado mydf
rm(mydf)
```
...
```{r}
#mostramos la variable cran
cran
#podemos observar como nos muestra una tabla
#usamos la funcion glimpse
glimpse(cran)
#ya no nos muestra los datos en forma de tabla sino que nos lo enseña por filas 
```
# dplyr
...
```{r}
filter(cran, r_version == "3.5.1", package == "plotly")
```
...
```{r}
avg_bytes <- summarise(cran, avg = mean(size))
```
# Agrupamientos
...
```{r}
 cran %>%
  group_by(package) %>%
  summarize(descarga = n()) %>%
  ungroup() %>%
  mutate(porcentaje = quantile(descarga, probs = 0.99)) %>%
  filter(descarga > porcentaje)
...
top_counts_sorted <- cran %>%
                        ...
                        summarize(count = n()) %>%
                        ungroup() %>%
                        mutate(porcentaje = quantile(count, probs = 0.99)) %>%
                        filter(count > porcentaje)%>%
                        arrange(desc(count))
...
top_unique_sorted <- cran %>%
  group_by(package) %>%
  summarize(unique = n_distinct(ip_id)) %>%
  ungroup() %>%
  mutate(porcentaje = quantile(unique, probs = 0.99)) %>%
  filter(unique > porcentaje) %>%
  arrange(desc(unique))
